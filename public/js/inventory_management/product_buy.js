(function () {
    ...
    const INITIAL = window.INVENTORY_DATA || { items: [], planDate: '', history: [], typeImport: 'fg_receipt_purchase' };
    const TYPE_IMPORT = INITIAL.typeImport || 'fg_receipt_purchase';
    ...
    // Diễn giải riêng cho hàng thương mại: "Nhập kho <sp> từ NCC <ncc> ngày dd/mm/yyyy"
    function buildInterpretation(name, supplier, date) {
        const s = supplier && supplier.trim() !== '' ? supplier : '(chưa có NCC)';
        return 'Nhập kho ' + name + ' từ NCC ' + s + ' ngày ' + date;
    }
    ...
    const runSearch = debounce(function () {
        ...
        postForm('search_product_supplier', { keyword: kw }).then(res => {
    ...
    function renderDropdown(items) {
        ...
        activeIdx = -1;
        if (!items.length) {
            $dropdown.innerHTML = '<li class="empty">Không tìm thấy sản phẩm thương mại</li>';
        } else {
            $dropdown.innerHTML = items.map((it, i) => {
                const sup = it.supplier_name ? ' — ' + escapeHtml(it.supplier_name) : '';
                return `<li data-id="${it.id}" data-idx="${i}">${escapeHtml(it.product_name)}${sup}</li>`;
            }).join('');
        }
    ...
    function selectDropdownItem(el) {
        ...
        postForm('get_product_supplier', { product_id: id }).then(res => {
    ...
    function addProductItem(p) {
        ...
        const interp = (p.interpretation && p.interpretation !== '')
            ? p.interpretation
            : buildInterpretation(name, supplier, date);
    ...
    $btnRec.addEventListener('click', () => {
        ...
        if (!confirm('Ghi nhập kho (thương mại) hiện tại vào tồn kho?')) return;
