        ...
        <div class="content">
            <div class="data-actions" data-sticky-top>
                <form class="data-filter" method="get">
                    ...
                    <input type="hidden" name="action" value="sale_inventory_issue_data">
                    ...
                    <input type="hidden" name="goods"       value="<?php echo htmlspecialchars((string) ($goods ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
            ...
            <div class="datatable-wrap" data-paginate-wrap>
                <div class="datatable-toolbar" data-sticky-top>
                    <div class="rows-per-page">
                        <label for="rpp-sii">Số hàng hiển thị:</label>
                        <select id="rpp-sii" data-rows-per-page>
                ...
                <table class="product-table" data-paginate data-sticky-thead>
                    <thead>
                        <tr>
                            <th>
                                <div class="wp-title">
                                    <div class="title">Tên hàng hóa</div>
                                    <div class="btn-filter" data-filter-target="goods"
                                         data-search-type="goods" data-filter-title="Lọc tên hàng hóa">
                    ...
                    <tbody>
                        <?php
                        $tot_qty = 0.0; $tot_amt = 0.0;
                        if (empty($rows)): ?>
                            <tr class="empty-row"><td colspan="7">Chưa có dữ liệu.</td></tr>
                        <?php else: foreach ($rows as $r):
                            $tot_qty += (float) $r['quantity'];
                            $tot_amt += (float) $r['amount'];
                            $is_material = !empty($r['material_id']);
                            if ($is_material) {
                                $hname = $r['material_name'] ?: ('#' . (int) $r['material_id']);
                            } else {
                                $hname = $r['product_name'] ?: ('#' . (int) $r['product_id']);
                            }
                            $cname = $r['customer_name'] ?: ($r['customer_id'] ? ('#' . (int) $r['customer_id']) : '');
                        ...
                        ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($cname, ENT_QUOTES, 'UTF-8'); ?></td>
                                <td class="center"><?php echo htmlspecialchars($wh, ENT_QUOTES, 'UTF-8'); ?></td>
