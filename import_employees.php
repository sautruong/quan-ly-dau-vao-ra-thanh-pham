...
{
    $conn = new mysqli("localhost", "root", "", "safeking_erp");
    ...
    foreach ($rows as $rIndex => &$row) {
        // Loại bỏ các ô rỗng ở cuối dòng
        $row = array_filter($row, fn($v) => $v !== null && $v !== '');
        $row = array_values($row); // 🔥 QUAN TRỌNG
    }
    // echo "<pre>";
    // print_r($rows);
    // echo "</pre>";
...
// gọi hàm
get_data_from_excel("product_categories.xlsx", "product_categories");
